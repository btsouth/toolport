...
- Replace the explicit checksum placeholder with the immutable `v1.18.0` source
  archive checksum after that tag exists.
- Run the stable, RC, and pinned-edge Omarchy beta plus GNOME Wayland, KDE
  Wayland, and X11 matrix for tray behavior, keyring prompts, fractional
  scaling, accessibility, and long-running resource use.
